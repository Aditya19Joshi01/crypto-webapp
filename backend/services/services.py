...
CELO_REGISTRY_ADDRESS = os.getenv("CELO_REGISTRY_ADDRESS", "0x000000000000000000000000000000000000ce10")
CUSD_ADDRESS = os.getenv("CUSD_ADDRESS", "0x765DE816845861e75A25fCA122bb6898B8B1282a")

logger = logging.getLogger("backend.services")

async def fetch_coingecko_price(client: httpx.AsyncClient, coin_id: str) -> Optional[float]:
...
async def fetch_defillama_tvl(client: httpx.AsyncClient, protocol: str):
    try:
        ...
        resp.raise_for_status()
        data = resp.json()
        logger.debug(f"DeFiLlama {protocol} TVL fetched with {len(data) if isinstance(data, list) else 'obj'} entries")
        return data
    except Exception:
...
def _fetch_cusd_price_blocking() -> Optional[float]:
    ...
    try:
        ...
        registry = w3.eth.contract(address=Web3.to_checksum_address(CELO_REGISTRY_ADDRESS), abi=[
            {"inputs":[{"internalType":"string","name":"identifier","type":"string"}],"name":"getAddressForString","outputs":[{"internalType":"address","name":"addr","type":"address"}],"stateMutability":"view","type":"function"}
        ])
        ...
        if int(sorted_oracles_addr, 16) == 0:
            ...
            return None
        sorted_oracles = w3.eth.contract(address=sorted_oracles_addr, abi=[
            {"inputs":[{"internalType":"address","name":"token","type":"address"}],"name":"medianRate","outputs":[{"internalType":"uint256","name":"numerator","type":"uint256"},{"internalType":"uint256","name":"denominator","type":"uint256"}],"stateMutability":"view","type":"function"}
        ])
